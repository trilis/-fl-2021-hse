...
intersect_regex(const std::shared_ptr<Regex> &left,
                const std::shared_ptr<Regex> &right) {
...
static std::shared_ptr<Regex> union_regex(std::shared_ptr<Regex> left,
                                          std::shared_ptr<Regex> right) {
...
static std::shared_ptr<Regex> nullable(const std::shared_ptr<Regex> &reg) {
  ...
  if (Concat * concat; concat = dynamic_cast<Concat *>(reg.get())) {
    return std::shared_ptr<Regex>(intersect_regex(
        nullable(concat->get_left()), nullable(concat->get_right())));
  }

  if (Alt * alt; alt = dynamic_cast<Alt *>(reg.get())) {
    return std::shared_ptr<Regex>(
        union_regex(nullable(alt->get_left()), nullable(alt->get_right())));
  }
...
derivative(const std::shared_ptr<Char> &left_character,
  ...
  if (Alt * alt; alt = dynamic_cast<Alt *>(reg.get())) {
    return std::shared_ptr<Regex>(
        new Alt(derivative(left_character, alt->get_left()),
                derivative(left_character, alt->get_right())));
  }
  ...
  if (Concat * concat; concat = dynamic_cast<Concat *>(reg.get())) {
    return std::shared_ptr<Regex>(
        new Alt(new Concat(derivative(left_character, concat->get_left()),
                           concat->get_right()),
                new Concat(nullable(concat->get_left()),
