...
shared_ptr<Regexp> der(char h, shared_ptr<Regexp> r) {
    ...
    if (r->type == "Alt"){
        ...
        return make_shared<Alt>(der(h,c->l),der(h,c->r));
    }
    if (r->type == "Concat"){
        Concat *c = dynamic_cast<Concat *>(r.get());
        return make_shared<Alt>(make_shared<Concat>(der(h,c->l),c->r),make_shared<Concat>(nullable(c->l),der(h,c->r)));
    }
    if (r->type == "Star"){
        Star *c = dynamic_cast<Star *>(r.get());
        return make_shared<Concat>(der(h,c->r),make_shared<Star>(*c));
    }
...
bool match(shared_ptr<Regexp> r, string s) {
    return (*nullable(der_s(s, r))).type == "Epsilon";
}
